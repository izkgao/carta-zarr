...
// The end of a block of selected channels that begins at `begin` and would like to be `desired`
// long, moved so that it lands on a spectral chunk boundary.
//
// Splitting inside a chunk would make one decode contribute to two blocks, and the walk decodes
// each chunk exactly once. When the desired length does not reach the end of the chunk it started
// in, the block grows to that chunk's end rather than shrinking to nothing.
std::uint64_t AlignedBlockEnd(std::uint64_t begin, std::uint64_t desired, std::uint64_t total,
                              std::uint64_t axis_start, std::uint64_t stride, std::uint64_t chunk) {
    if (desired == 0) {
        desired = 1;
    }
    const std::uint64_t target = std::min(begin + desired, total);
    if (target >= total || chunk == 0) {
        return target;
    }
    const auto chunk_of = [&](std::uint64_t index) { return (axis_start + (index * stride)) / chunk; };
    // The first selected index at or after an absolute channel.
